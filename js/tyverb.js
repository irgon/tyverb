...
$(document).ready(function() {
    ...
    /* LEFT SLIDER */
    if($('div.column-left div.items').is('div')) {
        $('div.column-left div.items > div').hide();
        $('div.column-left div.items > div.' + $('div.column-left ul.points li.selected').attr('class').match(/p[0-9]+/)[0]).show();
        $('div.column-left ul.points li').click(function(e) {
            e.preventDefault();
            if(!$(this).hasClass('selected')) {
                $('div.column-left ul.points li').removeClass('selected');
                $('div.column-left div.items > div:visible').fadeOut();
                $('div.column-left div.items div.' + $(this).attr('class').match(/p[0-9]+/)[0]).fadeIn();
                $('div.column-left ul.points li.' + $(this).attr('class').match(/p[0-9]+/)[0]).addClass('selected');
            }
        })
    }
    ...
    /* MAIN SLIDER */
    if($('#slider').is('div')) {
        $('#slider div.banner > div').hide();
        $('#slider div.banner > div.' + $('#slider ul.dots li.selected').attr('class').match(/b[0-9]+/)[0]).show();
        $('#slider ul.dots li').click(function(e) {
            e.preventDefault();
            if(!$(this).hasClass('selected')) {
                $('#slider ul.dots li').removeClass('selected');
                $('#slider div.banner > div:visible').fadeOut(1000);
                $('#slider div.banner div.' + $(this).attr('class').match(/b[0-9]+/)[0]).fadeIn(1000);
                $('#slider ul.dots li.' + $(this).attr('class').match(/b[0-9]+/)[0]).addClass('selected');
            }
        });
        setInterval(function() {
            main_nr = parseInt($('#slider ul.dots li.selected').attr('class').match(/b[0-9]+/)[0].replace('b', ''));
            main_nr = (main_nr == 3 ? 1 : main_nr + 1);
            $('#slider ul.dots li.b' + main_nr.toString()).click();
        }, 5000);
    }
